...
async def upload_vpn_config(
    ...
    """Upload VPN configuration for OpenVPN or Fortinet SSL VPN."""
    try:
        ...
        if connection_type == 'openvpn':
            if not file:
                raise HTTPException(status_code=400, detail="OpenVPN requires a .ovpn file upload.")
        ...
        elif connection_type == 'fortinet_ssl':
            if not host or not port:
                raise HTTPException(status_code=400, detail="Fortinet SSL VPN requires host and port.")
            # Create VPN config entry
            vpn_config = VPNConfig(
                organization_id=org_id,
                user_email=user_email,
                name=f"Fortinet SSL VPN {host}:{port}",
                connection_type='fortinet_ssl',
                config_metadata={
                    "host": host,
                    "port": port,
                    "username": username,
            ...
            db.commit()
            db.refresh(vpn_config)
            # Store in AWS (no file, just metadata)
            aws_result = await secrets_manager.store_vpn_config(
                str(vpn_config.config_id),
                "fortinet_ssl_placeholder_config",
                {"username": username, "password": password, "host": host, "port": port}
            )
        else:
            raise HTTPException(status_code=400, detail="Unsupported VPN connection type. Use 'openvpn' or 'fortinet_ssl'.")
...
async def connect_vpn(
    ...
    try:
        ...
        if config.connection_type == 'openvpn':
            ...
            # Use config.config_metadata directly (already has host/port/username/password)
            pass  # No need to overwrite or update config_metadata here
        else:
            raise HTTPException(status_code=400, detail="Unsupported VPN connection type")
        # Connect to VPN
